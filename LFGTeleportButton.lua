...
-- Development guard: if run outside the WoW client (e.g., generic Lua debugger),
-- the WoW API globals like CreateFrame/UIParent will be nil. In that case, stop here.
...
local DEFAULTS = {
  ...
  fontSize = 25,
  activityMap = {}, -- user-editable mapping: activityID (number) -> DUNGEONS index (number)
}
...
-- TWW S3 M+ dungeons (kullanıcı verdi)
local DUNGEONS = {
...
-- Source: wago.tools GroupFinderActivity DB (Retail 11.2.0.62493)
-- Only entries whose FullName includes "(Mythic Keystone)" and correspond to our Season 3 dungeons
...
-- Spell knowledge helper (handles overrides if available on this client)
...
-- Overlay border (opsiyonel hafif kenar)
...
-- Text overlay (replaces tooltip): two lines centered on the button
...
-- Now that the frame exists, create a classic bright hover glow on the button itself
...
local function EnsureStandaloneOptions()
  ...
  ---@diagnostic disable-next-line: redundant-parameter
  local frame = CreateFrame("Frame", "TPB_StandaloneOptions", UIParent)
  ...
  if AceGUI_local and AceGUI_local.Create then
    ...
    ---@diagnostic disable-next-line: redundant-parameter
    sizeSliderStandalone = CreateFrame("Slider", "TPB_FontSizeSliderStandalone", frame, "OptionsSliderTemplate")
  ...
  frame:SetScript("OnShow", function()
    ...
    if sizeWidgetStandalone and sizeWidgetStandalone.SetValue then
      sizeWidgetStandalone:SetValue((TPB_DB and TPB_DB.fontSize) or DEFAULTS.fontSize)
      if sizeValueLabelStandalone then sizeValueLabelStandalone:SetText(tostring((TPB_DB and TPB_DB.fontSize) or DEFAULTS.fontSize)) end
...
local function InitOptionsPanel()
  ...
  optionsPanel = CreateFrame("Frame", "TPB_OptionsPanel")
